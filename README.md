...
![Latest Version](https://img.shields.io/maven-metadata/v?metadataUrl=https%3A%2F%2Fregistry.somethingcatchy.net%2Frepository%2Fmaven-releases%2Fdev%2Fgalena%2Fhats-1.20.1-core%2Fmaven-metadata.xml&label=latest%20version%20for%201.20.1)
...
````kotlin
dependencies {
    implementation(jarJar("dev.galena:hats-1.20.1-forge:${galena_hats_version}") {
        version {
...
````kotlin
dependencies {
    modImplementation(include("dev.galena:hats-1.20.1-fabric:${galena_hats_version}"))
}
````
...
```kotlin
forge {
    includesMod("dev.galena:hats-1.20.1-forge:${galena_hats_version}")
}

// OR

fabric {
    includesMod("dev.galena:hats-1.20.1-fabric:${galena_hats_version}")
}
```
