...
![Latest Version](https://img.shields.io/maven-metadata/v?metadataUrl=https%3A%2F%2Fregistry.somethingcatchy.net%2Frepository%2Fmaven-releases%2Fdev%2Fgalena%2Fhats-26.2-core%2Fmaven-metadata.xml&label=latest%20version%20for%2026.2)
...
````kotlin
dependencies {
    implementation(jarJar("dev.galena:hats-26.2-neoforge:${galena_hats_version}") {
        version {
...
````kotlin
dependencies {
    modImplementation(include("dev.galena:hats-26.2-fabric:${galena_hats_version}"))
}
````
...
```kotlin
forge {
    includesMod("dev.galena:hats-26.2-neoforge:${galena_hats_version}")
}

// OR

fabric {
    includesMod("dev.galena:hats-26.2-fabric:${galena_hats_version}")
}
```
